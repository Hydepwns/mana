defmodule Blockchain.TransactionPool do
  ...
  def handle_call({:get_next_nonce, address}, _from, state) do
    ...
    next_nonce = if highest_nonce >= 0 do
      highest_nonce + 1
    else
  ...
  def handle_cast({:remove_transactions, transaction_hashes}, state) do
    new_state = Enum.reduce(transaction_hashes, state, fn hash, acc ->
  ...
  def handle_info(:cleanup, state) do
    ...
    new_state = Enum.reduce(old_hashes, state, fn hash, acc ->
      remove_transaction_from_pool(acc, hash)
    end)
    ...
    final_state = if map_size(new_state.transactions) > @max_pool_size do
  ...
  defp calculate_intrinsic_gas(transaction) do
    ...
    # Add gas for data
    data_gas = if transaction.data do
      byte_size(transaction.data) * 16  # Simplified: 16 gas per byte
    else
      0
    end
  ...
  defp validate_transaction_size(raw_transaction) do
    size = if is_binary(raw_transaction) do
  ...
  defp get_sorted_transactions(state, limit) do
    transactions = state.by_gas_price
