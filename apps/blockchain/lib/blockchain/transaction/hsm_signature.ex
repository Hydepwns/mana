defmodule Blockchain.Transaction.HSMSignature do
    ...
    case hsm_available?() do
      true ->
        ...
        case SigningService.sign_hash(hash, context) do
          {:ok, result} -> {result.v, result.r, result.s}
          {:error, _reason} -> 
  ...
  Maintains full compatibility with original implementation.
  """
  @spec is_signature_valid?(integer(), integer(), integer(), integer(), max_s: atom()) :: boolean()
  ...
  """
  @spec sign_transaction(Transaction.t(), private_key | key_id, non_neg_integer() | nil) :: Transaction.t()
  ...
  def batch_sign_transactions(transactions, key_id, chain_id \\ nil) do
    case hsm_available?() do
      true ->
        ...
        results = Enum.map(transactions, fn tx ->
          case SigningService.sign_transaction_with_key(tx, key_id, chain_id) do
            {:ok, signed_tx} -> signed_tx
            {:error, reason} -> {:error, reason}
          end
    ...
    case hsm_available?() do
      true ->
        ...
        case generate_hsm_key(label, role) do
          {:ok, key_id} ->
            Logger.warn("HSM key generated. Original private key cannot be imported for security reasons.")
            Logger.warn("You will need to manually update any addresses/accounts to use the new HSM key.")
