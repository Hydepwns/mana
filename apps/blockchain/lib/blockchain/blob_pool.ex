defmodule Blockchain.BlobPool do
  ...
  # Pool configuration
  @max_blob_txs 1000
  @max_blobs_per_tx 6  # EIP-4844 limit
  @max_blob_gas_per_block 786_432  # EIP-4844 target * 2
  
  # State structure
  defstruct [
    :blob_transactions,     # %{hash => {blob_tx, blob_data_list}}
    :by_sender,            # %{sender => MapSet.new([hash])}
    :by_gas_fee,           # Ordered list by blob gas fee
    :blob_gas_used,        # Current blob gas in pool
    :verified_proofs,      # MapSet of verified KZG proofs
    :config
  ]
  ...
  defp verify_kzg_proofs(blob_data_list, state) do
    ...
    proof_hashes = Enum.map(blob_data_list, fn {_blob, _commitment, proof} ->
