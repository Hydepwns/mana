defmodule Blockchain.Transaction.BlobTest do
  # Changed to false for KZG setup
  ...
  describe "Blob Transaction (EIP-4844)" do
    ...
    test "serializes blob transaction correctly", %{blob_tx: blob_tx} do
      ...
      # chain_id
      assert Enum.at(serialized, 0) == <<1>>
      # nonce
      assert Enum.at(serialized, 1) == <<42>>
      # to address
      assert Enum.at(serialized, 5) == blob_tx.to
      # data
      assert Enum.at(serialized, 7) == "hello"
    end
    ...
    test "calculates total blob gas correctly", %{blob_tx: blob_tx} do
      ...
      multi_blob_tx = %{
        blob_tx
        | blob_versioned_hashes: List.duplicate(hd(blob_tx.blob_versioned_hashes), 3)
      }
  ...
  describe "KZG Integration" do
    ...
    test "rejects blob count mismatch" do
      blob_tx = %Blob{
        ...
        max_fee_per_blob_gas: 1_000_000_000,
        # 2 hashes
        blob_versioned_hashes: [:crypto.strong_rand_bytes(32), :crypto.strong_rand_bytes(32)],
        v: 0,
        r: 1,
        s: 2
      }
    ...
    defp generate_test_blob_data() do
      ...
      field_elements =
        for i <- 0..4095 do
          ...
          # Keep first byte < 0x74 to ensure < modulus
          base_value = rem(i, 116)
    ...
    defp generate_different_test_blob_data() do
      ...
      field_elements =
        for i <- 0..4095 do
          ...
          base_value = rem(i + 1, 116)
          # Different offset
          rest = for j <- 1..31, do: rem(i + j + 42, 256)
          [base_value | rest] |> :binary.list_to_bin()
        end
