defmodule Blockchain.Integration.ShanghaiCancunTest do
  ...
  describe "EIP-4844 Full Integration" do
    test "complete blob transaction lifecycle" do
      ...
      assert stats.total_blob_gas == 131_072  # 1 blob = 2^17 gas
    end
    ...
    test "multiple blob transaction with replacement by fee" do
      ...
      txs_and_data = for {nonce, fee} <- [{1, 1_000_000}, {2, 2_000_000}, {3, 3_000_000}] do
      ...
      assert fees == [3_000_000, 2_000_000, 1_000_000]  # Descending order
    end
    
    test "batch KZG verification performance" do
      # Generate multiple blobs for batch testing
      blob_count = 10
      batch_data = for i <- 1..blob_count do
      ...
      assert speedup > 0.5  # Batch should be at least 50% as fast as individual
    end
  end
  ...
  describe "Performance Integration" do
    ...
    test "run comprehensive KZG benchmarks" do
      ...
      assert single_blob_perf.ops_per_sec > 1  # Should be able to do at least 1 op/sec
      assert single_blob_perf.avg_time_us < 10_000_000  # Should complete within 10 seconds
    end
  ...
  describe "Error Handling and Edge Cases" do
    test "invalid blob data handling" do
      ...
      invalid_blob = :crypto.strong_rand_bytes(1000)  # Wrong size
    ...
    test "blob pool limits and eviction" do
      ...
      for i <- 1..fill_count do
        ...
        tx = %Blob{
          ...
          max_fee_per_gas: 2_000_000_000,
          gas_limit: 21_000,
          to: <<i::160>>,  # Different address for each
          value: 0,
          data: "",
          access_list: [],
          max_fee_per_blob_gas: 1_000_000 + i,  # Increasing fees
          blob_versioned_hashes: [versioned_hash],
          v: 27,
          r: i,
          s: 2
      ...
      stats = BlobPool.stats()
      assert stats.total_transactions <= 1000  # Pool limit
    end
    ...
    test "KZG proof verification edge cases" do
      ...
      bad_commitment = :crypto.strong_rand_bytes(32)  # Wrong size
  ...
  defp generate_integration_test_blob(seed \\ 0) do
    ...
    field_elements = for i <- 0..(KZG.field_elements_per_blob() - 1) do
      ...
      base_value = rem(i + seed, 116)  # Keep first byte < 0x74
