defmodule Blockchain.PropertyTests.TransactionPropertyTest do
  ...
  property_test "transaction size limits" do
    check all tx <- transaction() do
      ...
      assert byte_size(serialized) < 1_000_000, "Transaction too large: #{byte_size(serialized)} bytes"
  ...
  fuzz_test "transaction serialization robustness", 
            &fuzz_serialize/1, 
            transaction(),
            max_runs: 500
  ...
  performance_test "transaction serialization performance",
  ...
  defp invalid_rlp_data() do
    frequency([
      ...
      {1, constant([1, 2, 3])},                      # Too few elements
      {1, list_of(binary(), min_length: 15, max_length: 20)}, # Too many elements
      {2, list_of(one_of([binary(), integer(), atom(:alphanumeric)]), length: 9)}, # Mixed types
      {1, constant("not a list")}                     # Not a list at all
    ])
  end
  ...
  property_test "transaction ordering by nonce and gas price" do
    check all txs <- list_of(transaction(), min_length: 2, max_length: 20) do
      ...
      grouped = Enum.group_by(txs, fn tx -> 
  ...
  property_test "gas economics invariants" do
    check all tx <- transaction() do
      ...
      if tx.gas_limit >= intrinsic_cost do
        ...
        assert total_fee >= tx.gas_limit  # No overflow
        assert total_fee >= tx.gas_price  # No overflow
      end
    end
  end
