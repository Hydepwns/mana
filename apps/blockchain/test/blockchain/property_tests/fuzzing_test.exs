defmodule Blockchain.PropertyTests.FuzzingTest do
  ...
  @moduletag :fuzzing
  @moduletag timeout: 300_000  # 5 minutes for intensive fuzzing

  # Transaction Processing Fuzzing

  fuzz_test "transaction processing with malformed inputs",
            &fuzz_transaction_processing/1,
            malformed_transaction(),
            max_runs: 1000,
            crash_on_error: false

  fuzz_test "transaction pool manipulation",
            &fuzz_transaction_pool/1,
            transaction_pool_operations(),
            max_runs: 500,
            crash_on_error: false

  fuzz_test "extreme value transaction handling",
  ...
  fuzz_test "block validation with corrupted data",
            &fuzz_block_validation/1,
            corrupted_block(),
            max_runs: 500,
            crash_on_error: false

  fuzz_test "chain reorganization stress test",
            &fuzz_chain_reorganization/1,
            chain_reorg_scenario(),
            max_runs: 200

  fuzz_test "uncle block processing",
  ...
  fuzz_test "EVM execution with random bytecode",
            &fuzz_evm_execution/1,
            random_evm_execution(),
            max_runs: 2000,
            crash_on_error: false

  fuzz_test "account state manipulation",
            &fuzz_account_state/1,
            account_state_operations(),
            max_runs: 500

  fuzz_test "storage trie corruption handling",
  ...
  fuzz_test "P2P message malformation",
            &fuzz_p2p_messages/1,
            malformed_p2p_message(),
            max_runs: 1000,
            crash_on_error: false

  fuzz_test "consensus message fuzzing",
  ...
  fuzz_test "reentrancy attack simulation",
            &fuzz_reentrancy_attacks/1,
            reentrancy_scenario(),
            max_runs: 200

  fuzz_test "integer overflow/underflow testing",
            &fuzz_integer_operations/1,
            extreme_integer_operations(),
            max_runs: 1000

  fuzz_test "gas exhaustion scenarios",
  ...
  defp malformed_transaction() do
    frequency([
      ...
      # Mixed data types
      {2, list_of(one_of([binary(), integer(), atom(:alphanumeric), list_of(binary())]), length: 9)},
  ...
  defp extreme_value_transaction() do
    gen all base_tx <- transaction(),
            extreme_nonce <- frequency([
              {1, constant(0)},
              {1, integer(1..1000)},
              {1, constant((1 <<< 256) - 1)},  # Max uint256
              {1, integer((1 <<< 255)..(1 <<< 256) - 1)}  # Very large
            ]),
            extreme_gas_price <- frequency([
              {1, constant(0)},
              {2, integer(1..1_000_000_000)},  # 1 Gwei
              {1, integer(1_000_000_000_000..1_000_000_000_000_000)},  # Very high
              {1, constant((1 <<< 256) - 1)}
            ]),
            extreme_gas_limit <- frequency([
              {1, constant(0)},
              {1, constant(21_000)},  # Minimum for transfer
              {2, integer(21_000..30_000_000)},  # Block gas limit range
              {1, integer(30_000_000..100_000_000)},  # Above block limit
              {1, constant((1 <<< 256) - 1)}
            ]),
            extreme_value <- frequency([
              {2, constant(0)},
              {2, integer(1..1000)},
              {1, wei_amount()},
              {1, constant((1 <<< 256) - 1)}
  ...
  defp malformed_signature_transaction() do
    gen all base_tx <- transaction(),
            malformed_r <- frequency([
              {1, constant(0)},
              {1, constant(1)},
              {1, integer(2..100)},
              {1, constant((1 <<< 256) - 1)}
            ]),
            malformed_s <- frequency([
              {1, constant(0)},
              {1, constant(1)},
              {1, integer(2..100)},
              {1, constant((1 <<< 256) - 1)}
            ]),
            malformed_v <- frequency([
              {2, integer(0..26)},
              {2, integer(29..255)},
              {1, integer(256..1000)}
            ]) do
  ...
  defp invalid_address_transaction() do
    gen all base_tx <- transaction(),
            invalid_to <- frequency([
              {2, binary(min_length: 0, max_length: 19)},  # Too short
              {2, binary(min_length: 21, max_length: 50)}, # Too long
              {1, constant("0x1234567890abcdef")},          # String instead of binary
              {1, integer(0..1000)}                         # Integer instead of binary
            ]) do
  ...
  defp corrupted_valid_block() do
    gen all base_block <- block(),
            corruption_type <- frequency([
  ...
  defp extreme_transaction() do
    gen all tx <- transaction() do
      ...
      extreme_scenarios = [
        %{tx | data: :crypto.strong_rand_bytes(1_000_000)},  # 1MB data
        %{tx | gas_limit: (1 <<< 64) - 1},                   # Max gas
        %{tx | gas_price: (1 <<< 256) - 1},                  # Max gas price
        %{tx | value: (1 <<< 256) - 1},                      # Max value
        %{tx | nonce: (1 <<< 64) - 1}                        # Max nonce
      ]
  ...
  defp chain_reorg_scenario() do
    gen all chain_length <- integer(3..20),
            fork_point <- integer(1..chain_length - 2),
            fork_length <- integer(1..10) do
  ...
  defp uncle_block_scenario() do
    gen all main_block <- block(),
            uncle_blocks <- list_of(block(), min_length: 1, max_length: 2),
            invalid_uncles <- list_of(corrupted_block(), min_length: 0, max_length: 3) do
  ...
  defp random_evm_execution() do
    gen all bytecode <- binary(min_length: 0, max_length: 2000),
            gas_limit <- integer(0..10_000_000),
            call_data <- binary(min_length: 0, max_length: 1000),
            call_value <- integer(0..(1 <<< 64)) do
  ...
  defp storage_corruption_scenario() do
    gen all address <- ethereum_address(),
            corruptions <- list_of(storage_corruption(), min_length: 1, max_length: 10) do
  ...
  defp storage_corruption() do
    frequency([
      {1, {:invalid_key, binary(min_length: 0, max_length: 31)}},  # Wrong key length
      {1, {:invalid_value, binary(min_length: 33, max_length: 100)}}, # Wrong value length
      {1, {:null_key, nil}},
      {1, {:null_value, nil}},
  ...
  defp malformed_p2p_message() do
    frequency([
      ...
      # Oversized messages
      {1, binary(min_length: 16_777_216, max_length: 17_000_000)}, # > 16MB
      
      # Invalid message IDs
      {2, gen all msg_id <- integer(256..1000),
                  data <- binary(min_length: 0, max_length: 100) do
            %{message_id: msg_id, data: data}
          end},
      
      # Malformed RLP in message data
      {1, gen all msg_id <- integer(0..255),
                  data <- invalid_rlp_structure() do
            %{message_id: msg_id, data: data}
          end}
    ])
  end
  ...
  defp malformed_block_announcement() do
    gen all block <- corrupted_block(),
            total_difficulty <- frequency([
              {1, integer(-1000..-1)},  # Negative
              {1, constant(0)},
              {2, integer(1..1000)},
              {1, constant((1 <<< 256) - 1)}  # Max
            ]) do
  ...
  defp corrupted_peer_discovery() do
    gen all node_id <- frequency([
              {1, binary(min_length: 0, max_length: 63)},  # Too short
              {1, binary(min_length: 65, max_length: 128)}, # Too long
              {2, binary(length: 64)}  # Correct length
            ]),
            ip <- frequency([
              {1, {256, 0, 0, 1}},     # Invalid IP
              {1, {0, 0, 0, 0}},       # Invalid IP
              {1, "192.168.1.1"},      # String instead of tuple
              {2, tuple([integer(0..255), integer(0..255), integer(0..255), integer(0..255)])}
            ]),
            port <- frequency([
              {1, integer(-1000..-1)}, # Negative port
              {1, constant(0)},        # Zero port
              {1, integer(65536..100000)}, # Too high
              {2, integer(1024..65535)} # Valid range
            ]) do
  ...
  defp malformed_protocol_handshake() do
    gen all protocol_version <- frequency([
              {1, integer(-10..-1)},   # Negative
              {1, constant(0)},        # Zero
              {2, integer(1..10)},     # Valid range
              {1, integer(100..1000)}  # Too high
            ]),
            network_id <- frequency([
              {1, integer(-100..-1)},  # Negative
              {2, integer(1..100)},    # Valid
              {1, constant((1 <<< 64) - 1)}  # Very large
            ]),
            capabilities <- frequency([
              {1, []},                 # No capabilities
              {1, ["invalid"]},        # Invalid capability
              {2, ["eth/66", "eth/65"]}, # Valid
              {1, list_of(binary(), min_length: 0, max_length: 100)} # Random
            ]) do
  ...
  defp reentrancy_scenario() do
    ...
    gen all target_contract <- ethereum_address(),
            attacker_contract <- ethereum_address(),
            call_depth <- integer(1..50),
            attack_value <- integer(1..1000) do
  ...
  defp extreme_integer_operations() do
    gen all operation <- frequency([
              {1, :add}, {1, :sub}, {1, :mul}, {1, :div}, {1, :mod},
              {1, :exp}, {1, :and}, {1, :or}, {1, :xor}, {1, :shift_left}, {1, :shift_right}
            ]),
            a <- frequency([
              {1, constant(0)},
              {1, constant(1)},
  ...
  defp invalid_rlp_structure() do
    frequency([
      {1, constant([])},                              # Empty list
      {1, list_of(binary(), min_length: 1, max_length: 3)},  # Too few elements
      {1, list_of(binary(), min_length: 20, max_length: 50)}, # Too many elements
      {1, list_of(one_of([binary(), integer(), list_of(binary())]), min_length: 5, max_length: 15)}, # Mixed types
      {1, map_of(binary(), binary())},                # Map instead of list
      {1, atom(:alphanumeric)}                        # Atom instead of list
    ])
  end

  defp block_with_malformed_transactions() do
    gen all base_block <- block(),
            malformed_txs <- list_of(malformed_transaction(), min_length: 1, max_length: 20) do
  ...
  defp fuzz_transaction_pool(operations) do
    try do
      ...
      final_pool = Enum.reduce(operations, pool, fn operation, current_pool ->
  ...
  defp fuzz_chain_reorganization(scenario) do
    try do
      ...
      # Add original chain
      chain_with_blocks = add_blocks_to_chain(chain, 
        Enum.take(scenario.blocks, scenario.original_chain_length))
      
      # Attempt reorganization
      fork_blocks = Enum.drop(scenario.blocks, scenario.fork_point)
  ...
  defp fuzz_account_state(operations) do
    try do
      ...
      # Apply all operations
      final_state = Enum.reduce(operations, state, fn operation, current_state ->
  ...
  defp fuzz_storage_corruption(scenario) do
    try do
      ...
      state = State.empty_state()
      account_state = State.put_account(state, scenario.address, %{
        balance: 1000,
        nonce: 0,
        storage: %{},
        code: <<>>
  ...
  defp fuzz_reentrancy_attacks(scenario) do
    try do
      ...
      _result = simulate_reentrancy_attack(
        scenario.target,
        scenario.attacker,
        scenario.depth,
        scenario.value
  ...
  defp fuzz_gas_exhaustion(scenario) do
    try do
      ...
      remaining_gas = Enum.reduce(scenario.operations, scenario.initial_gas, fn operation, gas ->
