defmodule ExWire.CLI.InteractiveCLI do
  ...
  @type cli_state :: %{
    context: atom(),  # :mainnet | :testnet | :dev | :multi_dc
    history: list(),
  ...
  def handle_call({:auto_complete, partial_command}, _from, state) do
    completions = AutoComplete.get_completions(state.auto_complete, partial_command, state.context)
  ...
  def handle_cast({:switch_context, new_context}, state) do
    ...
    new_state = %{state | 
      context: new_context,
      state: %{state.state | context: new_context}
    }
  ...
  def handle_cast(:toggle_debug_mode, state) do
    ...
    new_state = %{state | 
      state: %{state.state | debug_mode: new_debug_mode}
    }
  ...
  defp execute_pipeline([command | rest], input, state) do
    ...
    # Execute command
    result = case Map.get(state.command_registry, cmd) do
      nil ->
  ...
  defp get_context_color(:mainnet), do: @colors.error    # Red for mainnet (be careful!)
  defp get_context_color(:testnet), do: @colors.warning  # Yellow for testnet
  defp get_context_color(:dev), do: @colors.success      # Green for dev
  defp get_context_color(:multi_dc), do: @colors.primary # Cyan for multi-datacenter
  defp get_context_color(_), do: @colors.info            # Blue for others
end
...
defmodule ExWire.CLI.AutoComplete do
  ...
  def init(_opts) do
    ...
    state = %{
      commands: [
        "help", "status", "block", "blocks", "tx", "account", "balance", "nonce",
        "debug", "trace", "profile", "gas", "contract", "call", "consensus",
        "replicas", "datacenter", "crdt", "sync", "context", "history",
        "export", "import", "monitor", "logs", "peers", "network", "routing",
        "perf", "memory", "cpu", "disk"
      ],
      recent_addresses: [],
      recent_tx_hashes: [],
      recent_block_hashes: []
