defmodule ExWire.Sync.SnapshotSupervisor do
  ...
  def get_system_status() do
    ...
    server_status =
      case Process.whereis(ExWire.Sync.SnapshotServer) do
        nil -> :stopped
        pid when is_pid(pid) -> if Process.alive?(pid), do: :running, else: :error
      end
    ...
    generation_stats =
      if generator_status == :running do
        try do
  ...
  def set_serving_enabled(enabled) do
    ...
    try do
      ...
      error ->
        Logger.warning("[SnapshotSupervisor] Failed to set generator serving: #{inspect(error)}")
    end
    ...
    try do
      ...
      error ->
        Logger.warning("[SnapshotSupervisor] Failed to set server serving: #{inspect(error)}")
    end
  ...
  def get_system_info() do
    ...
    serving_info =
      if status.server_status == :running do
        try do
    ...
    available_snapshots =
      if status.generator_status == :running do
        try do
  ...
  def init(opts) do
    ...
    Logger.info(
      "[SnapshotSupervisor] Starting snapshot system with serving_enabled=#{Keyword.get(opts, :serving_enabled, true)}"
    )
