defmodule ExWire.Sync.StateDownloader do
  ...
  @batch_size 384  # Nodes per request (Ethereum's standard)
  @request_timeout 15_000  # 15 seconds
  @max_retries 3
  @heal_check_interval 30_000  # Check for missing nodes every 30s
  ...
  @type t :: %__MODULE__{
    ...
    # Node tracking
    node_queue: :queue.queue(binary()),  # Nodes to download
  ...
  defp process_download_queue(state) do
    ...
    if available_slots > 0 and :queue.len(state.node_queue) > 0 do
      ...
      new_state = Enum.reduce(nodes_batch, state, fn nodes, acc ->
  ...
  defp send_node_request(state, nodes) do
    ...
    if length(peers) > 0 do
      ...
      new_status = Enum.reduce(nodes, state.node_status, fn node, status_map ->
  ...
  defp handle_node_data(hashes_to_nodes, _peer, state) do
    ...
    # Process each node
    {new_state, new_nodes} = Enum.reduce(hashes_to_nodes, {state, []}, fn {_node_hash, node_data}, {acc_state, acc_nodes} ->
  ...
  defp extract_child_hashes(decoded_node) when is_list(decoded_node) do
    ...
    case length(decoded_node) do
      17 -> # Branch node
        decoded_node
        |> Enum.take(16)  # First 16 elements are child hashes
        |> Enum.filter(&(byte_size(&1) == 32))  # Only 32-byte hashes
        
      2 -> # Extension or leaf node
  ...
  defp queue_new_nodes(state, new_nodes) do
    ...
    unknown_nodes = Enum.filter(new_nodes, fn node ->
    ...
    if length(unknown_nodes) > 0 do
      ...
      new_queue = Enum.reduce(unknown_nodes, state.node_queue, fn node, queue ->
        :queue.in(node, queue)
      end)
      
      new_status = Enum.reduce(unknown_nodes, state.node_status, fn node, status_map ->
  ...
  defp handle_request_timeout(request_ref, state) do
    case Map.get(state.active_requests, request_ref) do
      ...
      {nodes, _peer} ->
        ...
        {retry_nodes, failed_nodes} = Enum.split_with(nodes, fn node ->
          retries = Map.get(state.request_retries, node, 0)
          retries < @max_retries
        end)
        
        # Update retry counts
        new_retries = Enum.reduce(retry_nodes, state.request_retries, fn node, retries ->
          Map.update(retries, node, 1, &(&1 + 1))
        end)
        
        # Re-queue nodes that can be retried
        new_queue = Enum.reduce(retry_nodes, state.node_queue, fn node, queue ->
          :queue.in(node, queue)
        end)
        
        # Mark permanently failed nodes
        new_status = retry_nodes
  ...
  defp perform_heal_check(state) do
    ...
    if length(missing_nodes) > 0 do
      ...
      new_queue = Enum.reduce(missing_nodes, state.node_queue, fn node, queue ->
        :queue.in(node, queue)
      end)
      
      new_status = Enum.reduce(missing_nodes, state.node_status, fn node, status_map ->
        Map.put(status_map, node, :pending)
      end)
