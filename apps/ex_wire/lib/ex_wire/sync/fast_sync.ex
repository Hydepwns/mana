defmodule ExWire.Sync.FastSync do
  ...
  @pivot_distance 128  # Number of blocks behind head for pivot
  ...
  @request_timeout 10_000  # Timeout for requests in ms
  @state_sync_start_delay 5_000  # Delay before starting state sync
  ...
  defp queue_header_downloads(state, from_block, to_block) do
    ...
    headers_to_queue = from_block..to_block
  ...
  defp queue_recent_block_downloads(state, from_block) do
    ...
    body_queue = Enum.reduce(blocks_to_queue, %{}, fn block_num, queue ->
      Map.put(queue, block_num, :pending)
    end)
    
    receipt_queue = Enum.reduce(blocks_to_queue, %{}, fn block_num, queue ->
      Map.put(queue, block_num, :pending)
    end)
  ...
  defp process_header_queue(state) do
    ...
    pending_headers = state.header_queue
  ...
  defp process_body_queue(state) do
    ...
    pending_bodies = state.body_queue
    |> Enum.filter(fn {_, status} -> status == :pending end)
    |> Enum.take(@max_concurrent_requests - map_size(state.active_requests))
  ...
  defp process_receipt_queue(state) do
    ...
    pending_receipts = state.receipt_queue
    |> Enum.filter(fn {_, status} -> status == :pending end)
    |> Enum.take(@max_concurrent_requests - map_size(state.active_requests))
  ...
  defp process_state_queue(state) do
    ...
    if MapSet.size(state.state_queue) > 0 and 
      ...
      nodes_to_download = state.state_queue
  ...
  defp download_state_nodes(state, nodes) do
    ...
    if length(peers) > 0 do
      ...
      new_queue = Enum.reduce(nodes, state.state_queue, fn node, queue ->
  ...
  defp handle_block_headers(headers, _peer, state) do
    ...
    new_state = Enum.reduce(valid_headers, state, fn header, acc ->
      # Store header
  ...
  defp handle_node_data(hashes_to_nodes, _peer, state) do
    ...
    Enum.reduce(hashes_to_nodes, state, fn {_hash, node}, acc ->
      ...
      new_queue = Enum.reduce(new_nodes, acc.state_queue, fn new_node, queue ->
  ...
  defp check_headers_complete(state) do
    all_complete = state.header_queue
    |> Map.values()
    |> Enum.all?(&(&1 == :complete))
  ...
  defp calculate_progress(state) do
    header_progress = if map_size(state.header_queue) > 0 do
