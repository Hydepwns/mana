defmodule ExWire.Eth2.BlobMetrics do
  ...
  defp init_operation_metrics do
    %{
      ...
      p95_ms: 0.0,
      p99_ms: 0.0,
      # Keep last 100 for percentile calculation
      recent_durations: [],
  ...
  defp update_operation_metrics(metrics, duration_ms, _value) do
    ...
    # Update min/max
    new_min =
      if duration_ms < metrics.min_duration_ms, do: duration_ms, else: metrics.min_duration_ms

    new_max =
      if duration_ms > metrics.max_duration_ms, do: duration_ms, else: metrics.max_duration_ms

    # Update recent durations for percentile calculation (keep last 100)
    recent_durations =
      [duration_ms | metrics.recent_durations]
      |> Enum.take(100)
  ...
  defp update_cache_metrics(cache, operation) do
    case operation do
      :cache_hit ->
        ...
        %{cache | total_hits: new_hits, hit_rate: hit_rate}
      ...
      :cache_miss ->
        ...
        %{cache | total_misses: new_misses, hit_rate: hit_rate}
  ...
  defp update_compression_metrics(compression, operation, value) do
    case operation do
      :store when value > 1.0 ->
        ...
        new_average =
          (compression.average_ratio * compression.compression_operations + value) /
            new_operations

        %{compression | compression_operations: new_operations, average_ratio: new_average}
  ...
  defp calculate_current_error_rate(metrics) do
    total_errors =
      metrics.errors.storage_errors + metrics.errors.compression_errors +
  ...
  defp format_compression_stats(compression) do
    ...
    savings_ratio =
      if compression.total_uncompressed_bytes > 0 do
