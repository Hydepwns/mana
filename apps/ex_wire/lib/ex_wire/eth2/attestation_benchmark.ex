defmodule ExWire.Eth2.AttestationBenchmark do
  ...
  def run_throughput_benchmarks(benchmark, opts) do
    ...
    results =
      Enum.map(attestation_counts, fn count ->
        attestations = create_test_attestations(beacon_state, count)

        # Benchmark parallel processing
        {parallel_time_us, parallel_results} =
        ...
        {sequential_time_us, sequential_results} =
  ...
  def run_batch_size_optimization(benchmark, opts) do
    ...
    results =
      Enum.map(batch_sizes, fn batch_size ->
        ...
        # Process in batches
        {total_time_us, batches_processed} =
  ...
  def run_concurrent_load_benchmark(benchmark, opts) do
    ...
    results =
      Enum.map(concurrent_clients, fn client_count ->
        # Create tasks for concurrent processing
        tasks =
          Enum.map(1..client_count, fn _ ->
            Task.async(fn ->
  ...
  defp benchmark_parallel_processing(attestations, beacon_state, fork_choice_store) do
    {:ok, processor} =
      ParallelAttestationProcessor.start_link(
  ...
  defp benchmark_sequential_processing(attestations, beacon_state, fork_choice_store) do
    ...
    :timer.tc(fn ->
      {valid, invalid} =
        Enum.reduce(attestations, {[], []}, fn attestation, {valid_acc, invalid_acc} ->
          # Simulate individual attestation processing time
          # 1ms per attestation (simulated)
          Process.sleep(1)
  ...
  defp create_test_attestations(beacon_state, count) do
    Enum.map(1..count, fn i ->
      # Vary slots within epoch
      slot = beacon_state.slot - rem(i, 32)

      %Attestation{
        aggregation_bits: generate_aggregation_bits(128),
        data: %{
          slot: slot,
          # Multiple committees per slot
          index: rem(i, 16),
  ...
  defp print_benchmark_results(benchmark) do
    ...
    IO.puts(
      "System: #{benchmark.system_info.schedulers} cores, Elixir #{benchmark.system_info.elixir_version}"
    )
    ...
    if throughput_results != [] do
      ...
      Enum.each(throughput_results, fn result ->
        IO.puts(
          String.pad_trailing("#{result.attestation_count}", 8) <>
    ...
    if batch_results != [] do
      ...
      IO.puts(
        String.pad_trailing("Batch Size", 12) <>
      ...
      Enum.each(batch_results, fn result ->
        IO.puts(
          String.pad_trailing("#{result.batch_size}", 12) <>
    ...
    if concurrent_results != [] do
      ...
      Enum.each(concurrent_results, fn result ->
        IO.puts(
          String.pad_trailing("#{result.client_count}", 8) <>
