defmodule ExWire.Eth2.PruningScheduler do
  ...
  @load_high_consensus_activity 500  # attestations/minute
  
  # Retry configuration
  @max_failures_before_pause 5
  @failure_backoff_base_ms 30_000    # 30 seconds
  @failure_backoff_max_ms 300_000    # 5 minutes
  ...
  def handle_cast({:cancel_job, job_id}, state) do
    ...
    active_jobs = if Map.has_key?(state.active_jobs, job_id) do
  ...
  def handle_call(:get_queue, _from, state) do
    queue_info = Enum.map(state.job_queue, fn job ->
      %{
        id: job.id,
  ...
  def handle_info(:incremental_cycle, state) do
    ...
    state = if should_run_incremental_pruning?(state) do
  ...
  def handle_info(:comprehensive_cycle, state) do
    ...
    state = if should_run_comprehensive_pruning?(state) do
  ...
  defp create_pruning_job(data_types, priority, opts) do
    %{
      ...
      timeout_ms: Map.get(opts, :timeout_ms, 300_000),  # 5 minutes default
      retries: 0,
  ...
  defp start_next_job(state) do
    ...
    if DateTime.diff(DateTime.utc_now(), job.scheduled_time, :millisecond) >= 0 do
      ...
      job_info = Map.put(job, :task_pid, task_pid)
    ...
    try do
      case job.data_types do
        ...
        data_types when is_list(data_types) ->
          results = Enum.map(data_types, fn data_type ->
  ...
  defp comprehensive_pruning_due?(state) do
    ...
    hours_since_prune >= 24  # Daily comprehensive pruning
  end
  ...
  defp adjust_scheduling_for_load(state, load) do
    case load do
      ...
      :high ->
        ...
        job_queue = Enum.map(state.job_queue, fn job ->
  ...
  defp get_scheduler_config(opts) do
    Map.merge(%{
      ...
      comprehensive_interval_ms: 3_600_000, # 1 hour
      load_monitoring_interval_ms: 30_000,  # 30 seconds
  ...
  defp schedule_incremental_cycle do
    Process.send_after(self(), :incremental_cycle, 60_000)  # 1 minute
  end
  
  defp schedule_comprehensive_cycle do
    Process.send_after(self(), :comprehensive_cycle, 3_600_000)  # 1 hour
  end
  
  defp schedule_load_monitoring do
    Process.send_after(self(), :load_monitoring, 30_000)  # 30 seconds
  end
end