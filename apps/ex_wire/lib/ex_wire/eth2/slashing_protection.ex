defmodule ExWire.Eth2.SlashingProtection do
  ...
  def handle_call({:check_block, pubkey, slot}, _from, state) do
    ...
    safe = not Enum.any?(blocks, fn record ->
      record.slot == slot
    end)
    
    # Check for lower slot (would be slashable if we sign)
    safe = safe and not Enum.any?(blocks, fn record ->
      record.slot > slot
    end)
  ...
  def handle_call({:export_interchange, pubkeys}, _from, state) do
    ...
    data = %{
      ...
      data: Enum.map(pubkeys, fn pubkey ->
        %{
  ...
  def handle_call({:import_interchange, data}, _from, state) do
    ...
    if data.metadata.interchange_format_version != "5" do
      ...
      state = Enum.reduce(data.data, state, fn validator_data, acc_state ->
  ...
  def handle_call({:prune, before_epoch}, _from, state) do
    ...
    attestations = Enum.map(state.attestations, fn {pubkey, records} ->
      pruned = Enum.filter(records, fn record ->
    ...
    # Prune old blocks
    blocks = Enum.map(state.blocks, fn {pubkey, records} ->
      pruned = Enum.filter(records, fn record ->
  ...
  defp build_config(opts) do
    %{
      ...
      save_interval: Keyword.get(opts, :save_interval, 60_000)  # 1 minute
    }
  end
