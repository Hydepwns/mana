defmodule ExWire.Eth2.CheckpointSync do
  ...
  @weak_subjectivity_period 54_000  # slots (~7.5 days)
  @backfill_batch_size 64  # blocks per request
  @backfill_rate_limit 10  # requests per second
  @state_download_timeout 60_000  # 1 minute
  @verification_depth 256  # blocks to verify after checkpoint
  ...
  def handle_info({:checkpoint_state_downloaded, checkpoint_state}, state) do
    ...
    if computed_root == state.checkpoint_root do
      ...
      Logger.error("State root mismatch! Expected: #{inspect(state.checkpoint_root)}, Got: #{inspect(computed_root)}")
  ...
  defp initiate_checkpoint_sync(checkpoint_root, checkpoint_epoch, state) do
    ...
    if peers == [] do
      ...
      %{state |
        ...
        backfill_progress: %{
          current_slot: checkpoint_epoch * 32,  # slots per epoch
          target_slot: 0,  # Genesis
          blocks_downloaded: 0,
          blocks_processed: 0,
  ...
  defp process_backfill_blocks(blocks, state) do
    ...
    processed = Enum.reduce(blocks, state, fn block, acc_state ->
  ...
  defp fetch_and_verify_blocks(start_slot, count) do
    ...
    peer = select_best_peer([])  # Get from available peers
