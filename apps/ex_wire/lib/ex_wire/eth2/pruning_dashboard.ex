defmodule ExWire.Eth2.PruningDashboard do
  ...
  defp create_overview_data(metrics_summary, pruning_status, system_health) do
    %{
      ...
      total_space_saved_gb: (Map.get(metrics_summary, :space_savings, %{})
                           |> Map.get(:total_space_freed_mb, 0)) / 1024,
      success_rate_percent: Map.get(metrics_summary, :operations, %{})
  ...
  defp create_performance_data(metrics_summary) do
    ...
    %{
      ...
      cpu_usage_percent: Map.get(metrics_summary, :system_impact, %{})
                       |> Map.get(:avg_cpu_usage_percent, 0),
      memory_usage_mb: Map.get(metrics_summary, :system_impact, %{})
                     |> Map.get(:avg_memory_usage_mb, 0),
      io_wait_percent: Map.get(metrics_summary, :system_impact, %{})
  ...
  defp evaluate_alerts(state) do
    ...
    all_alerts = (active_alerts ++ new_alerts)
  ...
  defp check_error_rate_alert(data, thresholds, alerts) do
    ...
    if error_rate > threshold do
      alert = %{
        ...
        threshold: threshold,
        timestamp: DateTime.utc_now(),
        expires_at: DateTime.add(DateTime.utc_now(), 600, :second)  # 10 minutes
      }
  ...
  defp slots_to_days(slots), do: slots / (32 * 225)  # ~32 slots/epoch, ~225 epochs/day
  ...
  defp schedule_dashboard_update do
    Process.send_after(self(), :update_dashboard, 5000)  # 5 seconds
  end
end