defmodule ExWire.Eth2.ForkChoiceOptimized do
  ...
  defstruct [
    ...
    children_cache: %{},      # parent -> list of children
    best_child_cache: %{},    # block -> best child
    best_descendant_cache: %{}, # block -> best descendant
    dirty_weights: MapSet.new(),  # blocks needing weight recalc
    pending_attestations: []   # buffer for batch processing
  ]
  ...
  @attestation_batch_timeout 100  # ms
  ...
  def on_attestation(store, attestation) do
    ...
    store = %{store | 
      pending_attestations: [attestation | store.pending_attestations]
    }
  ...
  def prune(store, finalized_root) do
    ...
    pruned_blocks = Map.filter(store.blocks, fn {root, _} ->
      MapSet.member?(blocks_to_keep, root) || root == finalized_root
    end)
    
    # Update caches
    pruned_weight_cache = Map.filter(store.weight_cache, fn {root, _} ->
      Map.has_key?(pruned_blocks, root)
    end)
    
    pruned_children = Map.filter(store.children_cache, fn {root, _} ->
      Map.has_key?(pruned_blocks, root)
    end)
    ...
    %{store |
      ...
      best_child_cache: %{},  # Will be rebuilt on demand
      best_descendant_cache: %{}  # Will be rebuilt on demand
    }
  end
  ...
  defp process_pending_attestations(store) do
    if store.pending_attestations == [] do
      ...
      attestations_by_target = Enum.group_by(
        store.pending_attestations,
        & &1.data.target.root
      )
      
      # Process each group
      store = Enum.reduce(attestations_by_target, store, fn {target_root, attestations}, acc ->
  ...
  defp process_attestation_batch(store, target_root, attestations) do
    ...
    total_weight = Enum.reduce(attestations, 0, fn attestation, acc ->
    ...
    latest_messages = Enum.reduce(attestations, store.latest_messages, fn attestation, acc ->
      validator_indices = get_attesting_indices(attestation, store)
  ...
  defp update_ancestor_weights(store, block_root, visited) do
    if MapSet.member?(visited, block_root) do
      ...
      child_weight = Enum.reduce(children, 0, fn child_root, acc ->
  ...
  defp update_best_descendant(store, block_root) do
    case Map.get(store.best_child_cache, block_root) do
      ...
      best_child ->
        ...
        %{store |
          best_descendant_cache: Map.put(store.best_descendant_cache, block_root, best_descendant)
        }
    end
  end
  ...
  defp get_best_descendant(store, block_root) do
    case Map.get(store.best_descendant_cache, block_root) do
      nil ->
        ...
        case Map.get(store.best_child_cache, block_root) do
          nil -> block_root  # No children, self is best
  ...
  defp get_current_slot(store) do
    div(store.time - store.genesis_time, 12)  # 12 seconds per slot
  end
