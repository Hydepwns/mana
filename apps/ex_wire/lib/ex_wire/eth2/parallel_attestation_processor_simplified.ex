defmodule ExWire.Eth2.ParallelAttestationProcessorSimplified do
  ...
  def process_attestations(attestations, beacon_state, fork_choice_store) do
    GenServer.call(__MODULE__, {:process_batch, attestations, beacon_state, fork_choice_store}, 30_000)
  end
  ...
  @impl true
  def init(opts) do
    Logger.info("Starting Simplified Parallel Attestation Processor with #{@max_parallel_workers} workers")
  ...
  def handle_cast({:queue_attestation, attestation, beacon_state, fork_choice_store}, state) do
    ...
    state = if length(pending) >= @batch_size do
  ...
  def handle_info(:process_batch, state) do
    ...
    state = if length(state.pending_attestations) > 0 do
      process_pending_batch(state)
    else
      state
    end
  ...
  defp process_attestations_parallel(attestations, beacon_state, fork_choice_store) do
    ...
    |> Flow.map(fn attestation ->
      ...
      with {:ok, attestation} <- validate_attestation_data(attestation, beacon_state),
           ...
           {:ok, attestation} <- validate_attestation_signature_simplified(attestation, committee, beacon_state),
  ...
  defp process_flow_results(results) do
    ...
    %{
      ...
      success_rate: if(length(valid) + length(invalid) > 0, 
  ...
  defp update_processing_stats(state, results, elapsed_ms) do
    ...
    avg_batch_time = if stats.total_processed == 0 do
      elapsed_ms
    else
  ...
  defp block_exists?(block_root) do
    ...
    rem(:erlang.phash2(block_root), 100) > 5  # 95% success rate
  end
