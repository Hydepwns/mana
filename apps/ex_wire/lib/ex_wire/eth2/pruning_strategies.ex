defmodule ExWire.Eth2.PruningStrategies do
  ...
  def prune_fork_choice(store, finalized_checkpoint, opts \\ []) do
    ...
    Logger.info("Starting enhanced fork choice pruning for finalized checkpoint #{finalized_checkpoint.epoch}")
    ...
    {blocks_to_prune, prune_reasons} = identify_prunable_blocks(
      store, 
  ...
  def prune_state_trie(state_store, retention_slots, opts \\ []) do
    ...
    {referenced_nodes, mark_time} = mark_referenced_nodes(state_store, retention_slots, parallel_marking)
    ...
    {compaction_stats, compact_time} = if compact_after do
  ...
  def prune_attestation_pool(attestation_pool, beacon_state, fork_choice_store, opts \\ []) do
    ...
    inclusion_window = if aggressive, do: 32, else: 64  # 1 or 2 epochs
    
    initial_count = count_attestations(attestation_pool)
    
    # Prune by age
    {pool_after_age, age_pruned} = prune_attestations_by_age(
      attestation_pool, 
      current_slot, 
      inclusion_window
    )
    ...
    {pool_after_orphan, orphan_pruned} = prune_orphaned_attestations(
      pool_after_age,
      fork_choice_store
    )
    
    # Prune invalid committee attestations
    {pool_after_committee, committee_pruned} = prune_invalid_committee_attestations(
      pool_after_orphan,
      beacon_state
    )
    
    # Optional deduplication
    {final_pool, dedup_pruned} = if deduplicate do
  ...
  def prune_block_storage(block_store, finalized_slot, config, opts \\ []) do
    ...
    if archive_mode do
      ...
      full_retention = finalized_slot - Map.get(config, :full_block_retention, 32 * 256)  # ~1 day
      header_retention = finalized_slot - Map.get(config, :header_retention, 32 * 256 * 7)  # ~1 week
      ...
      {store_after_archive, archive_stats} = if cold_storage do
      ...
      {final_store, removal_stats} = remove_ancient_blocks(store_after_archive, header_retention * 2)
  ...
  def prune_execution_logs(log_store, finalized_slot, config, opts \\ []) do
    ...
    retention_slots = Map.get(config, :execution_log_retention, 32 * 256 * 30)  # ~1 month
    ...
    important_logs = if keep_important do
  ...
  defp identify_prunable_blocks(store, finalized_descendants, current_slot, safety_margin) do
    ...
    Enum.reduce(store.blocks, {prunable, reasons}, fn {block_root, block_info}, {acc_blocks, acc_reasons} ->
      cond do
  ...
  defp execute_fork_choice_pruning(store, blocks_to_prune) do
    ...
    latest_messages = Map.filter(store.latest_messages || %{}, fn {_validator, message} ->
  ...
  defp prune_attestations_by_age(attestation_pool, current_slot, window_slots) do
    ...
    pruned_pool = Map.filter(attestation_pool, fn {slot, _attestations} ->
      slot >= cutoff_slot
    end)
  ...
  defp prune_orphaned_attestations(attestation_pool, fork_choice_store) do
    ...
    pruned_pool = Map.map(attestation_pool, fn {slot, attestations} ->
      {slot, Enum.filter(attestations, fn attestation ->
  ...
  defp deduplicate_attestations(attestation_pool) do
    ...
    deduped_pool = Map.map(attestation_pool, fn {slot, attestations} ->
  ...
  defp estimate_fork_choice_memory(store) do
    ...
    map_size(store.blocks) * 10_000  # ~10KB per block
  end
end