defmodule ExWire.P2P.PeerReputation do
  ...
  def update_for_successful_connection(reputation, duration_seconds) do
    ...
    # Update metrics
    new_metrics = %{reputation.metrics | 
  ...
  """
  @spec record_violation(reputation(), violation_type(), :low | :medium | :high | :critical, String.t()) :: reputation()
  ...
  def update_for_protocol_performance(reputation, performance_data) do
    ...
    new_metrics = %{reputation.metrics |
      response_time_avg: calculate_avg_response_time(reputation.metrics, performance_data.response_time),
      message_throughput: calculate_message_throughput(reputation.metrics, performance_data.message_count),
      bytes_transferred: reputation.metrics.bytes_transferred + performance_data.bytes_transferred
    }
    
    # Calculate performance score adjustment
    performance_score = calculate_performance_score(performance_data)
    score_adjustment = (performance_score - 0.5) * 0.02  # Small adjustment based on performance
  ...
  def cleanup_expired_violations(reputation) do
    ...
    active_violations = Enum.filter(reputation.violations, fn violation ->
      if violation.auto_expires do
        DateTime.diff(now, violation.timestamp, :second) < 86400  # 24 hours
      else
        true  # Keep non-auto-expiring violations
      end
    end)
  ...
  defp calculate_duration_bonus(duration_seconds) do
    cond do
      duration_seconds >= 3600 -> 1.5  # 1+ hour connection
      duration_seconds >= 1800 -> 1.2  # 30+ minute connection
      duration_seconds >= 600 -> 1.1   # 10+ minute connection
      duration_seconds >= 60 -> 1.0    # 1+ minute connection
      true -> 0.8  # Short connection
    end
  end

  defp calculate_avg_duration(metrics, new_duration) do
    if metrics.total_connections > 0 do
      ((metrics.avg_connection_duration * (metrics.total_connections - 1)) + new_duration) / metrics.total_connections
    else
      new_duration
    end
  end
  ...
  defp update_tags_for_violation(tags, violation_type, severity) do
    new_tags = case {violation_type, severity} do
  ...
  defp should_auto_expire?(violation_type, severity) do
    case {violation_type, severity} do
      {_, :critical} -> false  # Critical violations don't auto-expire
      {:malicious_blocks, _} -> false  # Malicious behavior doesn't auto-expire
      {:spam_behavior, :high} -> false  # High-severity spam doesn't auto-expire
      _ -> true  # Most violations auto-expire
    end
  end
  ...
  defp calculate_performance_score(performance_data) do
    ...
    response_score = max(0.0, min(1.0, 1.0 - (performance_data.response_time / 5000.0)))  # 5s timeout
    throughput_score = min(1.0, performance_data.message_count / 100.0)  # Normalize to 100 messages
  ...
  defp has_recent_critical_violations?(reputation) do
    ...
    Enum.any?(reputation.violations, fn violation ->
      ...
      DateTime.diff(now, violation.timestamp, :second) < 3600  # Within last hour
    end)
  end
end