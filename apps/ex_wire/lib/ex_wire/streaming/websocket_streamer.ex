defmodule ExWire.Streaming.WebSocketStreamer do
  ...
  def handle_cast({:broadcast_event, event_type, data, datacenter}, state) do
    ...
    case should_deduplicate_event(stream_event, state) do
      false ->
        ...
        new_state = %{state | 
          event_buffer: trimmed_buffer,
          deduplication_cache: new_cache
        }
  ...
  def handle_cast({:client_connected, client_id, socket, metadata}, state) do
    if map_size(state.active_connections) >= @max_connections do
      ...
      Logger.info("[WebSocketStreamer] Client #{client_id} connected from #{Map.get(metadata, :ip, "unknown")}")
    ...
    case Map.get(state.active_connections, client_id) do
      ...
      client_connection ->
        ...
        if length(valid_events) != length(event_types) do
          ...
          Logger.warn("[WebSocketStreamer] Invalid event types from client #{client_id}: #{inspect(invalid_events)}")
        end
        
        # Update client subscriptions
        updated_client = %{client_connection | 
        ...
        new_subscribers = update_event_subscribers(valid_events, client_id, state.event_subscribers)
        ...
        Logger.info("[WebSocketStreamer] Client #{client_id} subscribed to: #{inspect(valid_events)}")
  ...
  def handle_info(:heartbeat_check, state) do
    ...
    new_subscribers = remove_clients_from_subscribers(timed_out_client_ids, state.event_subscribers)
  ...
  def handle_info(:cleanup_deduplication, state) do
    ...
    if cleaned_count > 0 do
      Logger.debug("[WebSocketStreamer] Cleaned #{cleaned_count} entries from deduplication cache")
    end
    ...
    case Map.get(state.event_subscribers, event_type, []) do
      ...
      subscribers ->
        ...
        Enum.each(filtered_subscribers, fn client_id ->
          case Map.get(state.active_connections, client_id) do
            nil ->
              Logger.warn("[WebSocketStreamer] Subscriber #{client_id} not found in active connections")
        ...
        Logger.debug("[WebSocketStreamer] Broadcasted #{event_type} to #{length(filtered_subscribers)} clients")
    end
  end
  ...
  defp send_event_to_client(client_connection, stream_event) do
    ...
    Logger.debug("[WebSocketStreamer] Sending event to client #{client_connection.client_id}: #{String.slice(json_event, 0, 100)}...")
  end
  ...
  defp schedule_performance_monitoring() do
    Process.send_after(self(), :performance_monitoring, 60_000)  # Every minute
  end
  
  defp schedule_deduplication_cleanup() do
    Process.send_after(self(), :cleanup_deduplication, 300_000)  # Every 5 minutes
  end
end