defmodule ExWire.LibP2P.GossipSub do
  ...
  @d 8  # Target degree for mesh
  @d_low 6  # Lower bound for mesh degree
  @d_high 12  # Upper bound for mesh degree
  @d_lazy 6  # Degree for gossip emission
  @heartbeat_interval 1_000  # 1 second
  @fanout_ttl 60_000  # 1 minute
  @mcache_len 5  # Message cache history length
  @mcache_gossip 3  # Number of history windows to gossip
  @seen_ttl 120_000  # 2 minutes
  ...
  # Scoring parameters
  @p1 1.0  # Time in mesh weight
  @p2 1.0  # First message deliveries weight
  @p3 1.0  # Mesh message deliveries weight
  @p3b 0.5  # Mesh message deliveries threshold
  @p4 -1.0  # Invalid message weight
  @decay_interval 1_000  # 1 second
  @decay_to_zero 0.01
  ...
  defstruct [
    ...
    :d_high,
    :d_lazy,
    mesh: %{},  # topic => MapSet of peer_ids
    fanout: %{},  # topic => MapSet of peer_ids
    peers: %{},  # peer_id => peer_info
    topics: %{},  # topic => MapSet of interested peers
    mcache: [],  # Message cache (ring buffer)
    seen: %{},  # message_id => timestamp
    scores: %{},  # peer_id => score
    handlers: %{},  # topic => handler function
    config: %{}
  ]
  ...
  def handle_cast({:add_peer, peer_id, peer_info}, state) do
    ...
    new_peers = Map.put(state.peers, peer_id, Map.merge(peer_info, %{
      topics: MapSet.new(),
      score: 0.0,
  ...
  def handle_cast({:remove_peer, peer_id}, state) do
    ...
    new_mesh = Enum.reduce(state.mesh, %{}, fn {topic, peers}, acc ->
      Map.put(acc, topic, MapSet.delete(peers, peer_id))
    end)
    
    # Remove from fanout
    new_fanout = Enum.reduce(state.fanout, %{}, fn {topic, peers}, acc ->
      Map.put(acc, topic, MapSet.delete(peers, peer_id))
    end)
    
    # Remove from topics
    new_topics = Enum.reduce(state.topics, %{}, fn {topic, peers}, acc ->
      Map.put(acc, topic, MapSet.delete(peers, peer_id))
    end)
  ...
  def handle_info(:cleanup, state) do
    ...
    new_seen = Map.filter(state.seen, fn {_id, timestamp} ->
      now - timestamp < @seen_ttl
    end)
    
    # Clean up fanout
    new_fanout = Map.filter(state.fanout, fn {_topic, last_pub} ->
  ...
  defp join_mesh(topic, state) do
    ...
    if needed > 0 do
      ...
      new_mesh_peers = Enum.reduce(candidates, current_peers, fn peer_id, acc ->
  ...
  defp prune_peers(topic, count, state) do
    ...
    new_mesh_peers = Enum.reduce(to_prune, current_peers, fn peer_id, acc ->
      send_prune(peer_id, topic)
  ...
  defp handle_ihave(state, peer_id, ihave_messages) do
    ...
    wanted = Enum.flat_map(ihave_messages, fn %{topic: topic, message_ids: ids} ->
  ...
  defp update_scores(state) do
    ...
    new_scores = Enum.reduce(state.peers, %{}, fn {peer_id, peer_info}, acc ->
  ...
  defp calculate_peer_score(peer_id, peer_info, state, now) do
    # P1: Time in mesh
    p1_score = Enum.reduce(peer_info.time_in_mesh, 0.0, fn {_topic, time}, acc ->
      acc + @p1 * (now - time) / 1000
    end)
    
    # P2: First message deliveries
    p2_score = Enum.reduce(peer_info.first_message_deliveries, 0.0, fn {_topic, count}, acc ->
      acc + @p2 * count
    end)
    
    # P3: Mesh message deliveries
    p3_score = Enum.reduce(peer_info.mesh_message_deliveries, 0.0, fn {_topic, count}, acc ->
      if count >= @p3b do
        acc + @p3 * count
      else
        acc
