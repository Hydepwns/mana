defmodule ExWire.LibP2P do
  ...
  @target_peers 50
  @dial_timeout 10_000
  @gossip_d 8  # Desired outbound degree
  @gossip_d_low 6  # Lower bound for outbound degree
  @gossip_d_high 12  # Upper bound for outbound degree
  ...
  def init(opts) do
    ...
    {:ok, peer_manager} = PeerManager.start_link(
      max_peers: config.max_peers,
      target_peers: config.target_peers
    )
    
    # Initialize GossipSub
    {:ok, gossipsub} = GossipSub.start_link(
      node_id: node_id,
      d: @gossip_d,
      d_low: @gossip_d_low,
  ...
  def handle_info({:peer_disconnected, peer_id, reason}, state) do
    ...
    # Clean up streams
    new_streams = Map.reject(state.streams, fn {_id, stream} ->
  ...
  defp build_enr(node_id, config) do
    %{
      ...
      attnets: <<0xFF, 0xFF>>,  # Subscribed to all attestation subnets
      syncnets: <<0x00>>  # Not on sync committee
    }
  end
  ...
  defp schedule_peer_management do
    Process.send_after(self(), :manage_peers, 30_000)  # Every 30 seconds
  end
