defmodule ExWire.Eth2.ParallelAttestationProcessorSimplifiedTest do
  ...
  setup do
    ...
    {:ok, processor} = ParallelAttestationProcessorSimplified.start_link([
  ...
  describe "Flow-based parallel processing" do
    test "processes attestations in parallel using Flow", %{
      ...
      results = ParallelAttestationProcessorSimplified.process_attestations(
        attestations,
        beacon_state,
        fork_choice_store
      )
      ...
      assert elapsed_ms < 10_000  # 10 seconds max for 50 attestations
      ...
      assert throughput > 100  # At least 100 attestations/sec
    end
    ...
    test "handles different batch sizes efficiently", %{
      ...
      Enum.each(results, fn r ->
        IO.puts("#{r.batch_size}\t#{Float.round(r.time_ms, 1)}\t\t#{Float.round(r.throughput, 0)}\t\t#{Float.round(r.success_rate * 100, 1)}%")
      end)
    ...
    test "validates attestation data in parallel", %{
      ...
      assert length(results.invalid) > 0  # Should catch some invalid ones
      
      # Check that invalid reasons are properly categorized
      invalid_reasons = Enum.map(results.invalid, fn {reason, _} -> reason end)
      
      expected_reasons = [:future_slot, :invalid_target_epoch, :invalid_signature, :no_participants]
    ...
    test "scales processing across multiple CPU cores", %{
      ...
      IO.puts("Throughput per Core: #{Float.round(throughput / core_count, 0)} attestations/sec/core")
      IO.puts("===============================")
      
      # Should achieve reasonable per-core throughput
      per_core_throughput = throughput / core_count
      assert per_core_throughput > 50  # At least 50 per core
    ...
    test "maintains statistics correctly under load", %{
      ...
      IO.puts("Current Throughput: #{Float.round(final_stats.current_throughput, 0)} attestations/sec")
      IO.puts("=============================")
    end
  end
  ...
  defp create_test_attestations(beacon_state, count) do
    Enum.map(1..count, fn i ->
      slot = beacon_state.slot - rem(i, 10)  # Vary slots
      epoch = div(slot, 32)
    ...
    Enum.map(1..count, fn i ->
      cond do
        ...
        rem(i, 4) == 2 ->
          ...
          %Attestation{
            aggregation_bits: <<0::512>>,  # No participants
            data: %{
              slot: slot,
              index: 0,
              beacon_block_root: <<1::256>>,
        ...
        true ->
          ...
          %Attestation{
            ...
            data: %{
              slot: slot,
              index: 0,
              beacon_block_root: <<999::256>>,  # Unknown block
              source: %{epoch: 1, root: <<1::256>>},
              target: %{epoch: div(slot, 32), root: <<2::256>>}
            },
            signature: :crypto.strong_rand_bytes(96)
