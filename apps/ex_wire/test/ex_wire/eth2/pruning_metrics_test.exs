defmodule ExWire.Eth2.PruningMetricsTest do
  ...
  describe "space tracking" do
    ...
    test "calculates pruning efficiency correctly" do
      ...
      # 10 seconds
      PruningMetrics.record_operation(:states, :ok, 10000, %{})
      # 100 MB freed
  ...
  describe "optimization recommendations" do
    test "generates recommendations for poor performance" do
      ...
      # Very slow: 60s
      PruningMetrics.record_operation(:states, :ok, 60000, %{})
      # Low efficiency: 0.17 MB/s
      PruningMetrics.record_space_freed(:states, 10.0, 10.0, 0.0)
      # High system impact
      PruningMetrics.record_system_impact(90.0, 3000, 50.0, 1000)
    ...
    test "provides good health score for optimal performance" do
      ...
      # Fast: 5s
      PruningMetrics.record_operation(:states, :ok, 5000, %{})
      # Good efficiency: 20 MB/s
      PruningMetrics.record_space_freed(:states, 100.0, 100.0, 0.0)
      # Low system impact
      PruningMetrics.record_system_impact(10.0, 500, 5.0, 100)
  ...
  describe "error handling" do
    ...
    test "recovers gracefully from invalid data" do
      ...
      # Negative space freed
      assert :ok = PruningMetrics.record_space_freed(:invalid_type, -100.0, 0.0, 100.0)
