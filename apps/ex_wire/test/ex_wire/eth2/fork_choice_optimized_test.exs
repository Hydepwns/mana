defmodule ExWire.Eth2.ForkChoiceOptimizedTest do
  ...
  describe "Fork Choice Weight Caching" do
    ...
    test "attestation weights are cached and propagated", %{store: store} do
      ...
      store = Map.put(store, :pending_attestations, [])  # Force processing
    ...
    test "best child cache is maintained correctly", %{store: store} do
      ...
      store = %{store | 
        weight_cache: Map.put(store.weight_cache, block1a_root, 10)
      }
      store = %{store |
        weight_cache: Map.put(store.weight_cache, block1b_root, 5)
      }
      
      # Update best child cache
      store = Map.put(store, :best_child_cache, %{})  # Clear cache
  ...
  describe "Performance Comparison" do
    ...
    test "weight caching improves get_head performance" do
      # Create a large tree
      store = create_large_tree(100, 3)  # 100 blocks, branching factor 3
    ...
    test "batch attestation processing reduces overhead" do
      ...
      {batch_time, _} = :timer.tc(fn ->
        ...
        Map.put(store_with_pending, :pending_attestations, [])  # Force batch
      end)
    ...
    test "incremental cache updates are efficient" do
      ...
      deep_parent = <<50::256>>  # Assume this exists
  ...
  describe "Cache Correctness" do
    ...
    test "best descendant cache is accurate" do
      ...
      store = Enum.reduce(blocks, store, fn {block, root, weight}, acc ->
      ...
      head = ForkChoice.get_head(store)
      assert head in [<<4::256>>, <<6::256>>]  # Depending on weight calculation
    end
    ...
    test "dirty weight tracking prevents stale data" do
      ...
      # Trigger update
      store = Enum.reduce(dirty_blocks, store, fn block_root, acc ->
  ...
  defp create_aggregation_bits(indices) do
    ...
    bytes = for byte_idx <- 0..(byte_count - 1) do
      byte_bits = for bit_idx <- 0..7 do
  ...
  defp create_large_tree(block_count, branching_factor) do
    ...
    {store, _} = Enum.reduce(1..block_count, {store, [genesis_root]}, fn i, {acc_store, parent_roots} ->
