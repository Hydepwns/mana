defmodule ExWire.Eth2.PruningStressTest do
  ...
  @moduletag :stress
  @moduletag timeout: 600_000  # 10 minutes per test
  ...
  @stress_scenarios %{
    ...
    sustained_load: %{
      ...
      data_growth_rate: 100  # MB per minute
    }
  }
  ...
  setup_all do
    ...
    {:ok, _} = PruningMetrics.start_link(
      name: :stress_metrics,
  ...
  describe "extreme fork choice scenarios" do
    ...
    test "handles deep reorganizations with complex fork trees" do
      ...
      Logger.info("Fork probability: #{scenario.fork_probability * 100}%, Max reorg depth: #{scenario.max_reorg_depth}")
      ...
      for finalization_distance <- [50, 100, 500, 1000] do
        ...
        Logger.info("Distance #{finalization_distance}: #{pruned_blocks} blocks pruned (#{Float.round(pruning_ratio * 100, 1)}%) in #{duration_ms}ms")
    ...
    test "survives pathological fork structures" do
      ...
      pathological_scenarios = [
        create_many_short_forks(10_000, 3),      # Many 3-block forks
        create_long_competing_chains(5, 1000),   # 5 chains of 1000 blocks each
        create_dense_fork_pattern(1000, 0.8)    # Very high fork density
      ]
      ...
      for {scenario_name, fork_choice_data} <- pathological_scenarios do
        ...
        try do
          ...
          assert duration_ms < 120_000, "Should complete within 2 minutes even for pathological cases"
  ...
  describe "massive data volume stress" do
    ...
    test "handles massive attestation pools efficiently" do
      ...
      massive_dataset = generate_massive_attestation_data(
        scenario.validators,
      ...
      estimated_size_mb = total_attestations * 0.001  # ~1KB per attestation
      
      Logger.info("Generated #{total_attestations} attestations (~#{Float.round(estimated_size_mb, 1)} MB)")
      ...
      for {aggressive, dedupe, level_name} <- aggressiveness_levels do
        ...
        {:ok, pruned_pool, result} = PruningStrategies.prune_attestation_pool(
          massive_dataset,
          beacon_state,
          fork_choice_store,
        ...
        Logger.info("#{level_name}: #{result.total_pruned} attestations pruned in #{duration_ms}ms")
        Logger.info("Memory delta: #{Float.round(memory_delta_mb, 1)} MB")
        Logger.info("Throughput: #{Float.round(result.total_pruned / (duration_ms / 1000), 0)} attestations/sec")
    ...
    test "maintains performance under memory pressure" do
      ...
      concurrent_tasks = for task_id <- 1..scenario.concurrent_operations do
        Task.async(fn ->
          Logger.info("Starting concurrent stress task #{task_id}")
          
          # Each task processes a subset of the data
          subset_states = large_dataset.beacon_states.states
      ...
      memory_monitor = Task.async(fn ->
        monitor_memory_pressure(30_000)  # Monitor for 30 seconds
      end)
      
      # Wait for all tasks to complete
      task_results = Task.await_many(concurrent_tasks, 300_000)  # 5 minute timeout
      memory_stats = Task.await(memory_monitor, 35_000)
      
      # Analyze results under memory pressure
      total_duration = Enum.reduce(task_results, 0, fn {_id, _result, duration}, acc ->
        acc + duration
      end)
      ...
      Logger.info("Tasks: #{length(task_results)}, Average duration: #{Float.round(avg_duration, 0)}ms")
  ...
  describe "sustained load testing" do
    ...
    test "maintains performance under sustained high-frequency operations" do
      ...
      try do
        ...
        assert load_results.success_rate >= 0.95, "Should maintain 95%+ success rate under sustained load"
        assert load_results.avg_duration_ms < 30_000, "Average operation time should remain reasonable"
  ...
  describe "edge cases and error conditions" do
    ...
    test "handles corrupt or invalid data gracefully" do
      ...
      for {scenario_name, corrupt_data} <- corrupt_scenarios do
        ...
        try do
          result = PruningStrategies.prune_fork_choice(
            corrupt_data,
    ...
    test "handles resource exhaustion scenarios" do
      ...
      memory_limit = get_available_memory() * 0.8  # Use 80% of available memory
      ...
      try do
        ...
        case result do
          {:ok, _pruned_store, _result} ->
            ...
            assert duration_ms < 180_000, "Should complete within 3 minutes even under memory pressure"
  ...
  defp analyze_fork_complexity(fork_choice_data) do
    ...
    fork_points = Enum.count(parent_child_map, fn {_parent, children} ->
      length(children) > 1
    end)
    
    max_fork_width = parent_child_map
    |> Map.values()
    |> Enum.map(&length/1)
    |> Enum.max(fn -> 0 end)
  ...
  defp find_block_at_slot(blocks, target_slot) do
    ...
    |> case do
      [{root, _info} | _] -> root
      [] -> :crypto.strong_rand_bytes(32)  # Fallback
    end
  end
  ...
  defp verify_fork_choice_pruning_correctness(pruned_store, finalized_checkpoint) do
    ...
    for {_root, block_info} <- pruned_store.blocks do
      ...
      if parent_root != <<0::256>> do  # Skip genesis parent
  ...
  defp create_many_short_forks(count, fork_length) do
    ...
    # Create main chain
    main_chain = Enum.reduce(1..1000, {blocks, genesis_root}, fn slot, {acc_blocks, parent_root} ->
    ...
    fork_blocks = for i <- 1..count, reduce: blocks do
      acc_blocks ->
        # Pick random parent from main chain
        parent_candidates = Map.keys(acc_blocks) |> Enum.take_random(min(100, map_size(acc_blocks)))
  ...
  defp create_long_competing_chains(chain_count, chain_length) do
    ...
    chain_blocks = for chain_id <- 1..chain_count, reduce: blocks do
      acc_blocks ->
        # Each chain starts from genesis
        Enum.reduce(1..chain_length, {acc_blocks, genesis_root}, fn slot, {chain_acc, parent_root} ->
          block_root = :crypto.strong_rand_bytes(32)
          block_info = create_block_info(slot, parent_root)
  ...
  defp create_dense_fork_pattern(blocks_count, fork_density) do
    ...
    dense_blocks = Enum.reduce(1..blocks_count, blocks, fn slot, acc_blocks ->
      # Decide how many blocks to create for this slot
      blocks_at_slot = if :rand.uniform() < fork_density do
        :rand.uniform(5) + 1  # 1-5 blocks at this slot
      else
        1
      end
      
      # Pick parents from previous slots
      potential_parents = acc_blocks
  ...
  defp generate_massive_attestation_data(validators_count, attestations_per_slot, slots) do
    ...
    slot_batches = 0..(slots - 1)
    |> Enum.chunk_every(batch_size)
    
    all_attestations = slot_batches
    |> Enum.reduce(%{}, fn slot_batch, acc ->
      Logger.debug("Processing slot batch: #{List.first(slot_batch)}-#{List.last(slot_batch)}")
      
      batch_attestations = for slot <- slot_batch, into: %{} do
        attestations = generate_slot_attestations(slot, attestations_per_slot, validators_count)
        {slot, attestations}
      end
  ...
  defp monitor_memory_pressure_loop(start_time, duration_ms, samples) do
    ...
    if current_time - start_time >= duration_ms do
      ...
      new_samples = [current_memory | Enum.take(samples, 99)]  # Keep last 100 samples
  ...
  defp collect_sustained_metrics_loop(start_time, duration_ms, metrics) do
    ...
    if current_time - start_time >= duration_ms do
      ...
      new_metrics = [current_metric | Enum.take(metrics, 299)]  # Keep last 300 samples
  ...
  defp generate_sustained_pruning_load(manager_pid, duration_minutes, operations_per_minute) do
    ...
    interval_ms = round(60_000 / operations_per_minute)  # Milliseconds between operations
    
    Logger.info("Generating sustained load: #{operations_per_minute} ops/min for #{duration_minutes} minutes")
    
    results = for operation_id <- 1..total_operations do
      start_time = System.monotonic_time(:millisecond)
      ...
      |> tap(fn _ ->
        if rem(operation_id, 10) == 0 do
          Logger.debug("Completed #{operation_id}/#{total_operations} sustained load operations")
        end
  ...
  defp monitor_system_health_loop(start_time, duration_ms, samples) do
    ...
    if current_time - start_time >= duration_ms do
      if samples == [] do
        ...
        memory_stable = memory_variance < 0.2  # Less than 20% variance
      ...
      new_samples = [sample | Enum.take(samples, 59)]  # Keep last 60 samples
  ...
  defp calculate_throughput_variance(metrics_data) do
    throughputs = Enum.map(metrics_data, fn metric ->
  ...
  defp calculate_variance(values) do
    ...
    :math.sqrt(variance) / mean  # Coefficient of variation
  end
  ...
  defp create_corrupt_fork_choice_data(corruption_type) do
    ...
    case corruption_type do
      "missing_parent_references" ->
        blocks = %{
          :crypto.strong_rand_bytes(32) => create_block_info(1, :crypto.strong_rand_bytes(32))  # Parent doesn't exist
        }
      ...
      "circular_references" ->
        ...
        blocks = %{
          root1 => create_block_info(1, root2),  # Points to root2
          root2 => create_block_info(2, root1)   # Points back to root1 - circular!
        }
        {corruption_type, create_fork_choice_store(blocks)}
      ...
      "invalid_block_data" ->
        blocks = %{
          :crypto.strong_rand_bytes(32) => %{
            block: %{
              slot: -1,  # Invalid slot
              parent_root: "not_a_binary",  # Invalid parent root
              state_root: nil,  # Missing state root
              body_root: :invalid_data  # Wrong data type
            },
            root: :crypto.strong_rand_bytes(32),
            weight: "not_a_number",  # Invalid weight
            invalid: "not_a_boolean"  # Invalid invalid flag
          }
        }
        {corruption_type, create_fork_choice_store(blocks)}
      ...
      "inconsistent_weights" ->
        ...
        blocks = %{
          ...
          root2 => %{create_block_info(2, root1) | weight: 500}  # Child has less weight than parent
        }
        {corruption_type, create_fork_choice_store(blocks)}
    end
  end
  ...
  defp get_available_memory do
    ...
    total_memory * 4  # Assume we can use 4x current usage
  end
  ...
  defp generate_oversized_test_data(memory_limit_bytes) do
    ...
    estimated_block_size = 50_000  # ~50KB per block with all data
    ...
    large_blocks = Enum.reduce(1..target_blocks, blocks, fn slot, acc ->
      ...
      large_block_info = %{
        block: %{
          ...
          extra_data: :crypto.strong_rand_bytes(10_000)  # 10KB extra per block
        },
        root: block_root,
        weight: slot * 1000,
        invalid: false,
