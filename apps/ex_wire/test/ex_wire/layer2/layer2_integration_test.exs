defmodule ExWire.Layer2.IntegrationTest do
  ...
  describe "Full Layer 2 Integration" do
    test "complete optimistic rollup flow with sequencer and bridge" do
      ...
      {:ok, _rollup_pid} = OptimisticRollup.start_link(
        rollup_id: rollup_id,
        challenge_period: 100  # 100ms for testing
      )
      
      # 2. Start the sequencer
      {:ok, _seq_pid} = Sequencer.start_link(
        sequencer_id: sequencer_id,
        rollup_id: rollup_id,
        ordering_mode: :priority_fee,
        batch_size_limit: 10,
        batch_time_limit: 500  # 500ms for testing
      )
      
      # 3. Start the bridge
      {:ok, _bridge_pid} = CrossLayerBridge.start_link(
        bridge_id: bridge_id,
        l1_contract: <<1::160>>,
        l2_contract: <<2::160>>
      )
      ...
      transactions = for i <- 1..5 do
        {:ok, tx_hash} = Sequencer.submit_transaction(sequencer_id, %{
          from: <<i::160>>,
          to: <<100 + i::160>>,
          data: "test_data_#{i}",
          gas_limit: 21_000,
          priority_fee: i * 1_000_000_000,  # Increasing priority fees
      ...
      {:ok, deposit_id} = CrossLayerBridge.deposit(bridge_id, %{
        from: <<1::160>>,
        to: <<2::160>>,
        token: <<10::160>>,
      ...
      {:ok, message_id} = CrossLayerBridge.send_message(
        bridge_id,
        :l1,
        :l2,
        <<200::160>>,
      ...
      {:ok, challenge_id} = OptimisticRollup.challenge_state(
        rollup_id,
        1,
        batch.state_root
      )
      ...
      {:ok, result} = OptimisticRollup.submit_fraud_proof(
        rollup_id,
        challenge_id,
        fake_proof
      )
    ...
    test "complete ZK rollup flow with proof verification" do
      ...
      {:ok, _rollup_pid} = ZKRollup.start_link(
        rollup_id: rollup_id,
        proof_system: :plonk,
        state_tree_depth: 16
      )
      ...
      {:ok, proof_id} = ZKRollup.submit_proof(
        rollup_id,
        1,
        proof,
        public_inputs
    ...
    test "cross-layer bridge withdrawal flow" do
      ...
      {:ok, withdrawal_id} = CrossLayerBridge.withdraw(bridge_id, %{
        from: <<1::160>>,
        to: <<2::160>>,
        token: <<50::160>>,
      ...
      {:ok, msg_id} = CrossLayerBridge.send_message(
        bridge_id,
        :l2,
        :l1,
        <<100::160>>,
      ...
      {:ok, proof} = CrossLayerBridge.prove_message_inclusion(
        bridge_id,
        msg_id,
        state_root
      )
    ...
    test "sequencer MEV protection and ordering modes" do
      ...
      victim_tx = %{
        ...
        gas_limit: 100_000,
        priority_fee: 10_000_000_000,  # 10 Gwei
      ...
      frontrun_tx = %{
        ...
        data: "frontrun",
        gas_limit: 50_000,
        priority_fee: 50_000_000_000,  # 50 Gwei - higher fee
      ...
      backrun_tx = %{
        ...
        data: "backrun",
        gas_limit: 50_000,
        priority_fee: 5_000_000_000,  # 5 Gwei - lower fee
      ...
      :ok = Sequencer.update_config(sequencer_id, %{
        ordering_mode: :fifo,
    ...
    test "multi-rollup coordination" do
      ...
      {:ok, _} = OptimisticRollup.start_link(
        rollup_id: optimistic_id,
        challenge_period: 100
      )
      
      # Start ZK rollup
      {:ok, _} = ZKRollup.start_link(
        rollup_id: zk_id,
        proof_system: :stark,
        state_tree_depth: 20
      )
      ...
      assert opt_root != zk_root  # Different rollups have different states
    end
  end
