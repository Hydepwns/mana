defmodule VerkleTree.StateExpiryTest do
  ...
  setup do
    ...
    expiry_manager = StateExpiry.new(
      epoch_duration: 60,  # 1 minute epochs for testing
      active_epochs: 2,
      genesis_timestamp: System.system_time(:second) - 120  # Started 2 minutes ago
    )
  ...
  describe "get with expiry" do
    ...
    test "returns expired status for old state", %{tree: tree, expiry_manager: expiry_manager} do
      ...
      old_expiry_manager = %{expiry_manager | 
        genesis_timestamp: System.system_time(:second) - 7200  # 2 hours ago
      }
      ...
      future_expiry_manager = %{expiry_manager |
        genesis_timestamp: System.system_time(:second) - 7260  # Even older
      }
  ...
  describe "state resurrection" do
    test "resurrects expired state with valid proof", %{tree: tree, expiry_manager: expiry_manager} do
  ...
  describe "garbage collection" do
    test "removes expired nodes", %{tree: tree, expiry_manager: expiry_manager} do
      ...
      tree_with_data = Enum.zip(keys, values)
  ...
  describe "integration with standard operations" do
    test "expired state doesn't interfere with new writes", %{tree: tree, expiry_manager: expiry_manager} do
