defmodule MerklePatriciaTree.DB.ByzantineFaultTest do
  ...
  @moduletag timeout: 300_000  # 5 minutes for complex scenarios
  ...
  describe "Network Partition Scenarios" do
    test "handles conflicting updates during network partition", %{nodes: nodes, accounts: accounts} do
      ...
      assert balance_a == 1500  # 1000 + 500
      assert balance_b == 700   # 1000 - 300
      ...
      final_balances = Enum.map(nodes, fn node ->
    ...
    test "maintains consistency with multiple sequential partitions", %{nodes: nodes, accounts: accounts} do
      account = hd(accounts)
    ...
    test "recovers from total network fragmentation", %{nodes: nodes, accounts: accounts} do
      ...
      Enum.with_index(nodes) |> Enum.each(fn {node, index} ->
  ...
  describe "Clock Skew and Timestamp Attacks" do
    test "handles clock skew between nodes", %{nodes: nodes, accounts: accounts} do
      ...
      with_clock_skew(node_past, -3600, fn ->  # 1 hour behind
        set_account_balance(node_past, account, 1000)
      end)
      
      with_clock_skew(node_present, 0, fn ->  # Current time
        deposit(node_present, account, 500)
      end)
      
      with_clock_skew(node_future, 3600, fn ->  # 1 hour ahead
    ...
    test "prevents timestamp manipulation attacks", %{nodes: nodes} do
      ...
      future_timestamp = System.system_time(:millisecond) + 86_400_000  # 1 day in future
      
      result = with_timestamp(byzantine_node, future_timestamp, fn ->
  ...
  describe "State Divergence and Reconciliation" do
    ...
    test "handles concurrent state tree modifications", %{nodes: nodes} do
      ...
      all_keys = Enum.flat_map(nodes, fn node ->
  ...
  describe "CRDT Convergence Properties" do
    ...
    test "transaction pool CRDT handles duplicate submissions", %{nodes: nodes, transactions: transactions} do
      tx = hd(transactions)
      ...
      Enum.each(nodes, fn node ->
        ...
        count = Enum.count(pool, fn pool_tx -> 
    ...
    test "state tree CRDT resolves concurrent updates correctly", %{nodes: nodes} do
      ...
      tasks = Enum.with_index(nodes) |> Enum.map(fn {node, index} ->
        Task.async(fn ->
          Enum.each(1..10, fn i ->
