defmodule MerklePatriciaTree.StateManagementTest do
  ...
  setup do
    ...
    test_blocks = for i <- 1..200 do
      ...
      DB.put!(db, "block_#{i}", %{
        ...
        timestamp: System.os_time(:second) - (200 - i) * 12  # 12 second blocks
      } |> :erlang.term_to_binary())
      
      # Store state data
      DB.put!(db, state_root, :crypto.strong_rand_bytes(1024))  # 1KB state
  ...
  describe "State Pruning Modes" do
    ...
    test "full mode with reference counting", %{db: db} do
      ...
      active_roots = for i <- 150..200 do
        :crypto.hash(:sha256, <<"state_#{i}">>)
      end
      
      counter = Enum.reduce(active_roots, counter, fn root, acc ->
    ...
    test "archive mode performs no pruning", %{db: db} do
      ...
      {nodes_pruned, bytes_freed} = {0, 0}  # Archive mode behavior
  ...
  describe "Reference Counting System" do
    test "tracks references correctly", %{db: db, test_blocks: test_blocks} do
      ...
      counter = Enum.reduce(state_roots, counter, fn root, acc ->
        ReferenceCounter.increment(acc, root)
      end)
    ...
    test "identifies unreferenced nodes", %{db: db, test_blocks: test_blocks} do
      ...
      referenced_roots = test_blocks
                        |> Enum.take(100)
                        |> Enum.map(& &1.state_root)
      
      counter = Enum.reduce(referenced_roots, counter, fn root, acc ->
        ReferenceCounter.increment(acc, root)
      end)
  ...
  describe "Garbage Collection" do
    test "collects unreferenced nodes", %{db: db, test_blocks: test_blocks} do
      ...
      unreferenced_roots = test_blocks
                          |> Enum.take(50)
                          |> Enum.map(& &1.state_root)
    ...
    test "incremental collection respects time limits", %{db: db, test_blocks: test_blocks} do
      ...
      stats = GarbageCollector.incremental_collect(db, unreferenced_roots, 10)  # 10ms limit
      ...
      assert stats.duration_ms >= 0  # Could be 0 if very fast
    end
  ...
  describe "Long-running Node Scenarios" do
    @tag timeout: 120_000  # 2 minute timeout for long test
    test "handles extended operation with continuous state changes", %{db: db} do
      ...
      config = [
        ...
        gc_interval: 1000,        # 1 second for testing
        disk_usage_check_interval: 2000,  # 2 seconds
        full_pruning_interval: 5000       # 5 seconds
      ]
      
      {:ok, _pid} = StateManager.start_link(db, config)
      
      # Simulate continuous blockchain operation
      simulate_blockchain_activity(db, 300)  # 300 new blocks
    ...
    test "memory usage remains stable during long operation", %{db: db} do
      config = [
        pruning_mode: :full,
        gc_interval: 500,         # Aggressive GC for testing
      ...
      for batch <- 1..10 do
        ...
        Process.sleep(1000)  # Allow GC to run
    ...
    test "snapshot creation and restoration", %{db: db} do
      config = [pruning_mode: :archive, snapshot_interval: 86_400_000]  # Daily snapshots
      {:ok, _pid} = StateManager.start_link(db, config)
  ...
  describe "Performance Under Load" do
    test "maintains performance with large state trees", %{db: db} do
      ...
      for i <- 1..large_dataset_size do
        ...
        data = :crypto.strong_rand_bytes(2048)  # 2KB per entry
  ...
  defp simulate_blockchain_activity(db, num_blocks) do
    for i <- 1..num_blocks do
      ...
      state_data = :crypto.strong_rand_bytes(1024 + :rand.uniform(2048))  # Variable size
