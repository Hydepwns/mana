defmodule MerklePatriciaTree.StateManager.PruningPolicy do
  ...
  @type pruning_result :: {non_neg_integer(), non_neg_integer()}
  ...
  def prune_fast_mode(db, blocks_to_keep) do
    ...
    if current_block > blocks_to_keep do
      ...
      Logger.info(
        "[PruningPolicy] Fast pruning completed: #{nodes_removed} nodes, #{bytes_freed} bytes"
      )
  ...
  def prune_full_mode(db, reference_counter) do
    ...
    Logger.info(
      "[PruningPolicy] Full pruning completed: #{nodes_pruned} nodes, #{bytes_freed} bytes"
    )
  ...
  def emergency_prune(db, reference_counter, disk_usage_percentage) do
    Logger.warning(
      "[PruningPolicy] Emergency pruning triggered - disk usage: #{Float.round(disk_usage_percentage * 100, 1)}%"
    )
    ...
    Logger.warning(
      "[PruningPolicy] Emergency pruning completed: #{nodes_pruned} nodes, #{bytes_freed} bytes in #{duration}ms"
    )
  ...
  def get_pruning_recommendations(db, reference_counter) do
    ...
    {reasons, urgency_score, mode} =
      if length(unreferenced_nodes) > 10000 do
    ...
    # 10GB
    {reasons, urgency_score, mode} =
      if estimated_db_size > 10 * 1024 * 1024 * 1024 do
        new_mode =
          case urgency_score do
        ...
        {[
           "Database size too large (#{Float.round(estimated_db_size / (1024 * 1024 * 1024), 1)} GB)"
           | reasons
    ...
    {reasons, urgency_score, mode} =
      if current_block > 1000 do
        new_mode = if urgency_score < 30, do: :light, else: mode

        {["Large block count suggests pruning needed" | reasons], max(urgency_score, 40),
         new_mode}
      else
        {reasons, urgency_score, mode}
      end
    ...
    # Rough estimate
    estimated_bytes = length(unreferenced_nodes) * 1024
