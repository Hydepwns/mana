defmodule MerklePatriciaTree.DB.Migration.ETSToAntidote do
  ...
  """
  @spec migrate(ETS.db_ref(), Antidote.db_ref(), migration_opts()) :: {:ok, map()} | {:error, term()}
  def migrate(ets_ref, antidote_ref, opts \\ []) do
    ...
    case get_all_keys(ets_ref) do
      {:ok, keys} ->
        ...
        remaining_keys = if start_position > 0 do
  ...
  def verify_migration(ets_ref, antidote_ref, opts \\ []) do
    ...
    case get_all_keys(ets_ref) do
      {:ok, keys} ->
        ...
        mismatches = Enum.reduce(sampled_keys, [], fn key, acc ->
          ...
          case {ets_value, antidote_value} do
            ...
            {ets_val, ant_val} ->
              if verbose do
                Logger.warning("Mismatch for key #{inspect(key)}: " <>
  ...
  defp process_batches(keys, ets_ref, antidote_ref, batch_size, checkpoint_interval,
                      checkpoint_file, position, total, dry_run, verbose) do
    ...
    final_stats = Enum.reduce_while(chunks, stats, fn batch, acc ->
      batch_result = process_batch(batch, ets_ref, antidote_ref, dry_run, verbose)
      
      new_stats = case batch_result do
        {:ok, batch_stats} ->
          %{acc | 
