defmodule VerkleTree.Witness do
  ...
  defp collect_path_data(tree, key) do
    value = case VerkleTree.get(tree, key) do
      {:ok, val} -> val
      :not_found -> ""  # Use empty string instead of nil
    end
  ...
  defp collect_path_commitments(tree, key, current_commitment) do
    ...
    case DB.get(tree.db, current_commitment) do
      {:ok, encoded_node} ->
        ...
        case node do
          :empty -> []
          {:leaf, commitment, _value} -> [commitment]
          {:internal, children} ->
