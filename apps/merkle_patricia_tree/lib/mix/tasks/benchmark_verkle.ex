defmodule Mix.Tasks.Benchmark.Verkle do
  ...
  defp benchmark_verkle_witness(tree, test_data, config) do
    ...
    kvs_for_verification =
      test_data
      |> Enum.take(config.witness_size)
      |> Enum.map(fn {k, v} -> {k, v} end)

    {verify_time, _results} =
      :timer.tc(fn ->
  ...
  defp benchmark_verkle_migration(tree, config) do
    ...
    # Smaller dataset for migration
    test_data = generate_test_data(div(config.keys, 10))

    mpt =
      Enum.reduce(test_data, mpt, fn {key, value}, acc ->
        Trie.update_key(acc, key, value)
      end)

    # Benchmark migration initialization
    {init_time, migration_state} =
      :timer.tc(fn ->
  ...
  defp print_operation_results(name, result) do
    IO.puts(
      "  #{String.pad_trailing(name, 10)}: #{result.ops_per_second} ops/sec (#{result.avg_time_us} μs/op)"
    )
  end
  ...
  defp print_parallel_results(results) do
    ...
    Enum.each(results, fn result ->
      ...
      IO.puts(
        "    Verkle: #{result.verkle.ops_per_second} ops/sec (#{result.verkle.speedup}x speedup)"
      )
