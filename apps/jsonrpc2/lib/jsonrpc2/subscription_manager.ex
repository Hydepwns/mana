defmodule JSONRPC2.SubscriptionManager do
  ...
  def handle_call({:subscribe, subscription_type, params, ws_pid}, _from, state) do
    case validate_subscription_type(subscription_type) do
      :ok ->
        ...
        Logger.info("Created subscription #{subscription_id} of type #{subscription_type} for #{inspect(ws_pid)}")
  ...
  def handle_call({:unsubscribe, subscription_id, ws_pid}, _from, state) do
    case Map.get(state.subscriptions, subscription_id) do
      ...
      %{ws_pid: other_pid} ->
        Logger.warn("Unauthorized unsubscribe attempt for #{subscription_id} from #{inspect(ws_pid)}, owned by #{inspect(other_pid)}")
  ...
  def handle_info(:periodic_check, state) do
    ...
    if total_subs > 0 do
      Logger.debug("SubscriptionManager: #{total_subs} active subscriptions across #{total_connections} connections")
    end
  ...
  defp validate_subscription_type(type) when type in ["newHeads", "logs", "newPendingTransactions", "syncing"], do: :ok
