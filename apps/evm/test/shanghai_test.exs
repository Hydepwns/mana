defmodule EVM.ShanghaiTest do
  ...
  describe "EIP-1153: Transient Storage" do
    ...
    test "TSTORE and TLOAD basic functionality", %{
      ...
      result =
        StackMemoryStorageAndFlow.tload([key], %{
    ...
    test "TLOAD returns 0 for non-existent key", %{
      exec_env: exec_env,
      machine_state: machine_state
    } do
      key = 0x9999
    ...
    test "transient storage is isolated per contract", %{machine_state: machine_state} do
      ...
      exec_env1_updated = %{
        result1[:exec_env]
        | transient_storage: result2[:exec_env].transient_storage
      }
  ...
  describe "EIP-6780: SELFDESTRUCT changes" do
    ...
    test "SELFDESTRUCT only transfers balance post-Shanghai (not created in same tx)",
      ...
      account_repo =
        EVM.AccountRepo.repo(account_repo).add_account(
          account_repo,
          exec_env.address,
      ...
      result =
        EVM.Operation.System.selfdestruct([recipient], %{
          exec_env: exec_env,
          sub_state: sub_state
        })
  ...
  describe "Blob Base Fee Calculation" do
    ...
    test "calculate_blob_base_fee with excess gas" do
      ...
      # One target worth
      parent_excess = 393_216
      # One more target worth
      parent_used = 393_216
