defmodule ExthCrypto.HSM.ConfigManager do
  ...
  @default_security_policy %{
    ...
    max_signing_rate: 1000,  # signatures per minute
  ...
  @default_monitoring %{
    ...
    metrics_retention: 86_400  # 24 hours in seconds
  }
  ...
  def generate_sample_config(vendor) when vendor in [:thales, :aws_cloudhsm, :safenet_luna, :utimaco, :yubihsm, :softhsm] do
  ...
  defp load_from_environment() do
    ...
    # HSM_ENABLED
    env_config = if enabled = System.get_env("HSM_ENABLED") do
    ...
    # HSM_VENDOR
    env_config = if vendor = System.get_env("HSM_VENDOR") do
    ...
    # HSM_LIBRARY_PATH
    env_config = if library_path = System.get_env("HSM_LIBRARY_PATH") do
    ...
    # HSM_SLOT_ID
    env_config = if slot_id = System.get_env("HSM_SLOT_ID") do
    ...
    env_config = if pin = System.get_env("HSM_PIN") do
    ...
    # HSM_TOKEN_LABEL
    env_config = if token_label = System.get_env("HSM_TOKEN_LABEL") do
  ...
  defp validate_configuration(config) do
    ...
    errors = if not is_boolean(config[:enabled]) do
      ["enabled must be a boolean" | errors]
    else
      errors
    end
    
    errors = if config[:enabled] do
      # Validate HSM-specific configuration when enabled
      errors = if not is_atom(config[:vendor]) or config[:vendor] not in Map.keys(@vendor_defaults) do
        ["vendor must be one of: #{inspect(Map.keys(@vendor_defaults))}" | errors]
      else
        errors
      end
      
      errors = if not is_binary(config[:library_path]) do
        ["library_path must be a string" | errors]
      else
        errors
      end
      
      errors = if not is_integer(config[:slot_id]) or config[:slot_id] < 0 do
        ["slot_id must be a non-negative integer" | errors]
      else
        errors
      end
      
      errors = if config[:pin] && not is_binary(config[:pin]) do
      ...
      errors = if String.starts_with?(config[:library_path], "/") do
  ...
  defp validate_pool_config(pool_config, errors) do
    errors = if not is_integer(pool_config[:size]) or pool_config[:size] <= 0 do
      ["connection_pool.size must be a positive integer" | errors]
    else
      errors
    end
    
    errors = if not is_integer(pool_config[:max_overflow]) or pool_config[:max_overflow] < 0 do
      ["connection_pool.max_overflow must be a non-negative integer" | errors]
    else
      errors
    end
    
    errors = if not is_integer(pool_config[:timeout]) or pool_config[:timeout] <= 0 do
  ...
  defp validate_security_policy(security_policy, errors) do
    errors = if not is_list(security_policy[:require_hsm_for_roles]) do
      ["security_policy.require_hsm_for_roles must be a list" | errors]
    else
      errors
    end
    
    errors = if not is_boolean(security_policy[:allow_software_fallback]) do
