defmodule ExthCrypto.HSM.Monitor do
  ...
  @health_check_intervals %{
    ...
    key_manager: 60_000,
    # 45 seconds
    signing_service: 45_000,
    # 5 minutes
    config_manager: 300_000
  }

  # Performance monitoring intervals
  # 15 seconds
  @performance_check_interval 15_000

  # Alert thresholds
  @default_thresholds %{
    # 1 second
    response_time_warning: 1_000,
    # 5 seconds
    response_time_critical: 5_000,
    # 5%
    ...
    # 80% usage
    connection_pool_warning: 0.80,
    # 95% usage
    connection_pool_critical: 0.95
  }
  ...
  def handle_call(:get_alerts, _from, state) do
    response = %{
      ...
      # Last 100 alerts
      history: state.alert_history |> Enum.take(100)
    }
    ...
    case KeyManager.get_stats() do
      {:ok, stats} ->
        cond do
          ...
          stats.errors > stats.total_keys * 0.1 ->
    ...
    case KeyManager.get_stats() do
      {:ok, stats} ->
        ...
        %{
          ...
          # Would be calculated from actual timings
  ...
  defp collect_signing_service_metrics() do
    case SigningService.get_stats() do
      {:ok, stats} ->
        error_rate =
          if stats.total_signatures > 0, do: stats.errors / stats.total_signatures, else: 0.0

        pool_usage =
          if stats.config.max_concurrent_operations > 0 do
            stats.active_operations / stats.config.max_concurrent_operations
          else
            0.0
          end
  ...
  defp check_and_generate_alerts(component, health_result, state) do
    ...
    alerts_to_create =
      if health_result.response_time > @default_thresholds.response_time_critical do
        [
          create_alert(:critical, component, "High response time", %{
        ...
        if health_result.response_time > @default_thresholds.response_time_warning do
          [
            create_alert(:warning, component, "Elevated response time", %{
              response_time: health_result.response_time
            })
            | alerts_to_create
          ]
    ...
    alerts_to_create =
      case health_result.status do
        :unhealthy ->
          [
            create_alert(:critical, component, "Component unhealthy", %{
        ...
        :degraded ->
          [
            create_alert(:warning, component, "Component degraded", health_result.details)
            | alerts_to_create
          ]
    ...
    alerts_to_create =
      cond do
        metrics.error_rate > @default_thresholds.error_rate_critical ->
          [
            create_alert(:critical, component, "Critical error rate", %{
        ...
        metrics.error_rate > @default_thresholds.error_rate_warning ->
          [
            create_alert(:warning, component, "High error rate", %{error_rate: metrics.error_rate})
            | alerts_to_create
          ]
    ...
    alerts_to_create =
      cond do
        metrics.queue_length > @default_thresholds.queue_length_critical ->
          [
            create_alert(:critical, component, "Queue critically full", %{
        ...
        metrics.queue_length > @default_thresholds.queue_length_warning ->
          [
            create_alert(:warning, component, "Queue filling up", %{
              queue_length: metrics.queue_length
            })
            | alerts_to_create
          ]
    ...
    alerts_to_create =
      cond do
        metrics.connection_pool_usage > @default_thresholds.connection_pool_critical ->
          [
            create_alert(:critical, component, "Connection pool exhausted", %{
              pool_usage: metrics.connection_pool_usage
            })
            | alerts_to_create
          ]
        ...
        metrics.connection_pool_usage > @default_thresholds.connection_pool_warning ->
          [
            create_alert(:warning, component, "Connection pool under pressure", %{
              pool_usage: metrics.connection_pool_usage
            })
            | alerts_to_create
          ]
  ...
  defp calculate_performance_trends(history) do
    if length(history) < 2 do
      ...
      %{
        response_time_trend:
          calculate_trend(recent, [:metrics, :signing_service, :average_response_time]),
        error_rate_trend: calculate_trend(recent, [:metrics, :signing_service, :error_rate]),
        throughput_trend:
          calculate_trend(recent, [:metrics, :signing_service, :operations_per_second])
      }
    end
  end
  ...
  defp log_alert(alert) do
    case alert.severity do
      :critical ->
        Logger.error(
          "HSM_ALERT [CRITICAL] #{alert.component}: #{alert.message} - #{inspect(alert.details)}"
        )
      ...
      :info ->
        Logger.info(
          "HSM_ALERT [INFO] #{alert.component}: #{alert.message} - #{inspect(alert.details)}"
        )
